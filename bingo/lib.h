...
int bingo(Tabela * tab){
    for(int x = 0;x < 5;x++){
        for(int y = 0; y < 5; y++){
            if(tab->tabela[x][y] != 0){
                return 1;
            }if(tab->tabela[x][1] == 0 || tab->tabela[x][2] == 0 || tab->tabela[x][3] == 0 || tab->tabela[x][4] == 0 || tab->tabela[x][5] == 0){
                return 0;           
            }if(tab->tabela[y][1] == 0 || tab->tabela[y][2] == 0 || tab->tabela[y][3] == 0 || tab->tabela[y][4] == 0 || tab->tabela[y][5] == 0){
                return 0;           
            }
        }
    }
